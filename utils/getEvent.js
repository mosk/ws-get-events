...
const apikey = `zQTTHa5DNidTn8tGkZBn161614775`;
const getEvent = (evtString, id) => {
    return new Promise ((resolve, reject) => {
        ...
        let clearLink = evtString.replace(/\n/g, `~`).split(`~`)[0].replace(/Ссылка: /g, ``).replace(/ /g, ``);
        let clearPrice = evtString.replace(/\n/g, `~`).split(`~`)[4].replace(/Цена: /g, ``).replace(/руб./g, ``).replace(/  /g, ``);
        let clearDate = evtString.replace(/\n/g, `~`).split(`~`)[3].replace(/Дата: /g, ``).replace(/  /g, ``);
        let img = null;
        let ics = `ics#${id + 1}`;
